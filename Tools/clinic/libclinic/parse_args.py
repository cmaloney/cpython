...
class ParseArgsCodeGen:
    ...
    def _vectorcall_type_check(self) -> list[str]:
        """Assert `type` is a type this vectorcall was generated for.

        The generated code is only correct for a type whose tp_new / tp_init
        is the parser this vectorcall shadows: __init__ calls tp_new with no
        arguments, then the impl.  tp_vectorcall is not inherited, so
        subclasses never reach it; the assert catches C code installing the
        function on an unrelated type.  Checking the slot rather than the type
        object means heap types need no C pointer to compare against.
        """
        func = self.func
