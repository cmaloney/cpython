...
class ParseArgsCodeGen:
    ...
    def _vectorcall_exact_check(self) -> list[str]:
        """Start the vectorcall parser-code list.

        Contains the exact-type check if requested (with @vectorcall
        exact_only, subclasses fall back to the tp_new / tp_init slots),
    ...
    def _vectorcall_positional(self) -> list[str]:
        ...
        Used both for the all-pos-only case and for the no-keywords case
        of parse_vectorcall_pos_or_kw().
        """
        pos_code = self._parse_positional_args(
    ...
    def _assemble_vectorcall(self, preamble: str, fields: tuple[str, ...],
                             finale: str) -> None:
        """Wrap parser-code fields in the vectorcall prototype.

        Emit the prototype, then the preamble, the fields and the finale,
        mirroring parser_body().
        """
    ...
    def parse_vectorcall_no_args(self) -> None:
        # no parameters: check that no argument was passed and fall through
        # to the impl-call finale; no parse_args helper is needed.
    ...
    def parse_vectorcall_pos_only(self) -> None:
        # positional-only: parse positionally and fall through to the
        # impl-call finale; no parse_args helper is needed.
        parser_code = self._vectorcall_exact_check()
        self.codegen.add_include('pycore_modsupport.h',
                                 '_PyArg_NoKwnames()')
        parser_code.append(libclinic.normalize_snippet("""
    ...
    def _vectorcall_delegate_to_helper(self, nkw: str) -> str:
        ...
        'nkw' is the C expression for the number of keyword arguments.
        """
        if self.func.kind is METHOD_INIT:
    ...
    def parse_vectorcall_pos_or_kw(self) -> None:
        # Delegate to the parse_args helper when keywords are passed;
        # otherwise parse positionally, skipping _PyArg_UnpackKeywords,
        # and fall through to the impl-call finale.
        parser_code = self._vectorcall_exact_check()
    ...
    def parse_vectorcall_delegating(self) -> None:
        # Keyword-only / var-positional / var-keyword parameters: every
        # call is delegated to the parse_args helper, so the preamble
        # needs only nargs and the finale is just a closing brace.
        parser_code = self._vectorcall_exact_check()
    ...
    def parse_vectorcall(self) -> None:
        ...
        Mirrors parse_args(): dispatch on the parameter shape to a
        parse_vectorcall_*() method, each of which builds a list of
        parser-code snippets and wraps them in the prototype, preamble
        and finale.
        """
        if not self.parameters and not self.varpos and not self.var_keyword:
            ...
            self.parse_vectorcall_delegating()
        else:
