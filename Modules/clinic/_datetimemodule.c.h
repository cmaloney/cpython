...
static PyObject *
datetime_time_helper(PyTypeObject *type, PyObject *const *args,
    Py_ssize_t nargs, Py_ssize_t nkw, PyObject *kwargs, PyObject *kwnames)
{
    PyObject *return_value = NULL;
    #if defined(Py_BUILD_CORE) && !defined(Py_BUILD_CORE_MODULE)
...
datetime_time(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    {
        PyObject *pre_parse_result;
        int pre_parse_rc = time_pre_parse(type, _PyTuple_CAST(args)->ob_item,
            PyTuple_GET_SIZE(args),
            kwargs ? PyDict_GET_SIZE(kwargs) : 0,
            kwargs, NULL, &pre_parse_result);
        if (pre_parse_rc < 0) {
    ...
    return datetime_time_helper(type, _PyTuple_CAST(args)->ob_item,
        PyTuple_GET_SIZE(args),
        kwargs ? PyDict_GET_SIZE(kwargs) : 0,
        kwargs, NULL);
}
...
static PyObject *
datetime_time_vectorcall(PyObject *type, PyObject *const *args,
    size_t nargsf, PyObject *kwnames)
{
    Py_ssize_t nargs = PyVectorcall_NARGS(nargsf);

    assert(Py_Is(_PyType_CAST(type), &PyDateTime_TimeType));
    /* Make sure the type object is immutable: the generated
     * vectorcall doesn't deal e.g. with users reassigning __init__. */
    assert(PyType_HasFeature(_PyType_CAST(type), Py_TPFLAGS_IMMUTABLETYPE));
    {
        PyObject *pre_parse_result;
        int pre_parse_rc = time_pre_parse(_PyType_CAST(type), args, nargs,
            kwnames ? PyTuple_GET_SIZE(kwnames) : 0,
            NULL, kwnames, &pre_parse_result);
        if (pre_parse_rc < 0) {
            return NULL;
    ...
    return datetime_time_helper(_PyType_CAST(type), args, nargs,
        kwnames ? PyTuple_GET_SIZE(kwnames) : 0,
        NULL, kwnames);
}
...
static PyObject *
datetime_datetime_helper(PyTypeObject *type, PyObject *const *args,
    Py_ssize_t nargs, Py_ssize_t nkw, PyObject *kwargs, PyObject *kwnames)
{
    PyObject *return_value = NULL;
    #if defined(Py_BUILD_CORE) && !defined(Py_BUILD_CORE_MODULE)
    ...
    PyObject *argsbuf[9];
    PyObject * const *fastargs;
    Py_ssize_t noptargs = nargs + nkw - 3;
    int year;
    int month;
    int day;
    int hour = 0;
...
datetime_datetime(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    {
        PyObject *pre_parse_result;
        int pre_parse_rc = datetime_pre_parse(type, _PyTuple_CAST(args)->ob_item,
            PyTuple_GET_SIZE(args),
            kwargs ? PyDict_GET_SIZE(kwargs) : 0,
            kwargs, NULL, &pre_parse_result);
        if (pre_parse_rc < 0) {
    ...
    return datetime_datetime_helper(type, _PyTuple_CAST(args)->ob_item,
        PyTuple_GET_SIZE(args),
        kwargs ? PyDict_GET_SIZE(kwargs) : 0,
        kwargs, NULL);
}
...
static PyObject *
datetime_datetime_vectorcall(PyObject *type, PyObject *const *args,
    size_t nargsf, PyObject *kwnames)
{
    Py_ssize_t nargs = PyVectorcall_NARGS(nargsf);

    assert(Py_Is(_PyType_CAST(type), &PyDateTime_DateTimeType));
    /* Make sure the type object is immutable: the generated
     * vectorcall doesn't deal e.g. with users reassigning __init__. */
    assert(PyType_HasFeature(_PyType_CAST(type), Py_TPFLAGS_IMMUTABLETYPE));
    {
        PyObject *pre_parse_result;
        int pre_parse_rc = datetime_pre_parse(_PyType_CAST(type), args, nargs,
            kwnames ? PyTuple_GET_SIZE(kwnames) : 0,
            NULL, kwnames, &pre_parse_result);
        if (pre_parse_rc < 0) {
            return NULL;
    ...
    return datetime_datetime_helper(_PyType_CAST(type), args, nargs,
        kwnames ? PyTuple_GET_SIZE(kwnames) : 0,
        NULL, kwnames);
}
...
/*[clinic end generated code: output=32c4cfa527b4e26b input=a9049054013a1b77]*/
