...
/* Put a soft limit on the number of bytes to be written.
   ...
   In older versions of Windows a hard limit was necessary because there
   was a hard limit to the number of bytes (bpo-11395), but that is not
   the case in Windows 8+.

   For Winodws 8+ the console host synchronizes I/O operations such that
   a Ctrl-C doesn't generate an interrupt until after the write is
   completed which means very large write which may take multiple
   seconds reduce responsiveness.
