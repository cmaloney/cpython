...
bytearray_resize_lock_held(PyObject *self, Py_ssize_t requested_size)
{
    ...
    if (logical_offset > 0) {
        // Make a new non-offset value.
        size_t new_size = Py_MIN((size_t)requested_size, (size_t)Py_SIZE(self));
        PyObject *new = PyBytes_FromStringAndSize(
                PyByteArray_AS_STRING(self), new_size);
        if (new == NULL) {
            return -1;
        }
    }
        ...
        if (obj->ob_bytes_head == NULL) {
            obj->ob_bytes_head = PyBytes_FromStringAndSize(NULL, size);
        }
        else {
            // Was too small, make biggger.
            if (_PyBytes_Resize(&obj->ob_bytes_head, alloc) < 0) {
...
On error, bytearray will not be modified. If there are exports or the
bytearray data is offset will throw rather than copy.
[clinic start generated code]*/

static PyObject *
bytearray__detach_impl(PyByteArrayObject *self)
/*[clinic end generated code: output=eac03c9d6f8d6230 input=ae2b9e601a2f9e85]*/
{
    _Py_CRITICAL_SECTION_ASSERT_OBJECT_LOCKED(self);
    if (self->ob_exports > 0) {
        PyErr_SetString(PyExc_BufferError,
