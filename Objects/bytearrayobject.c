...
static int bytearray_take_existing(PyByteArrayObject *self, PyObject *bytes) {
    ...
    Py_SET_SIZE(self, size);
    FT_ATOMIC_STORE_SSIZE_RELAXED(self->ob_alloc, size + 1);
    Py_END_CRITICAL_SECTION();
    return 0;
}
...
/*[clinic end generated code: output=3147fbc0bbbe8d94 input=88b446df44ee88bc]*/
{
    ...
    // bytearray with exports may still change the bytes, no immutable bytes allowed.
    if (!_canresize(self)) {
        return NULL;
    }
    ...
    // Write remaining bytes, if any, to a new bytes.
